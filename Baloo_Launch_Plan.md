...
- **S4 ✅ write rate limits/caps shipped** (per-user limits on every community write + a 250-item
  ...
  **S6 ✅ error monitoring (Sentry) shipped** — guarded across server/edge/client, inert until M
  sets `SENTRY_DSN` + `NEXT_PUBLIC_SENTRY_DSN` on Vercel (source-map upload later, needs a Sentry auth
  token) · **S7a ✅ account deletion shipped** (GDPR erasure): migration `0008` made
